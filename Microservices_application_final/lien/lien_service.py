...
def check_domain_reputation(domain: str) -> dict:
    """Vérifier réputation via VirusTotal"""
    try:
...
def get_domain_age(domain: str) -> dict:
    """Obtenir l'âge du domaine via WHOIS"""
    try:
...
Règles :
- "coherent" si le texte affiché correspond au domaine de l'URL
- "suspect" si différence entre texte et URL, ou domaine suspect
...
def analyze_url(url: str) -> dict:
    ...
    # Calcul score pour cette URL
    url_score = 0
    
    # Réputation (+50 si non fiable)
    if reputation.get("status") == "not reliable":
        url_score += 50
    
    # Âge domaine (+50 si suspect ou inconnu)
...
def analyze_liens(analysis_id):
    """Analyse tous les liens d'un email"""
    print(f"\n🔗 LIEN - Analyse {analysis_id}")
    
    db = get_db()
    
    # Récupérer URLs depuis MySQL
    urls_data = db.get_urls(analysis_id)
    
    if not urls_data:
        print("  ℹ️  Aucune URL trouvée")
        return {
    ...
    # Score global : moyenne des scores d'URLs
    if url_scores:
        global_score = round(sum(url_scores) / len(url_scores), 2)
    else:
        global_score = 0
    
    # Explication
    if suspicious_count > 0:
        # Lister URLs suspectes
    ...
    print(f"  ⚠️  {suspicious_count} URL(s) suspecte(s)")
