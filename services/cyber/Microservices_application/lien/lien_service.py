...
def ai_analyze_link(display_text: str, raw_url: str, domain: str) -> dict:
    ...
    prompt = f"""
    Analyse ce lien et reponds STRICTEMENT en JSON sans ```:

    {{"status": "coherent" ou "suspect", "reason": "..."}}

    Texte affiche : {display_text}
    URL reelle    : {raw_url}
    Domaine       : {domain}
    """
    ...
    try:
        ...
        return {"status": "unknown", "reason": "Quota epuise"}
    except Exception as e:
        print(f"  Erreur IA: {e}")
...
def analyze_liens(analysis_id):
    ...
    if not use_groq:
        ...
        print("      Analyse basee uniquement sur VirusTotal + WHOIS")
    ...
    for url_entry in urls_data:
        ...
        analysis = analyze_url(url, use_groq=use_groq)
...
if __name__ == "__main__":
    ...
    print("   Scoring sur 100 | Groq skip si > 8 liens")
    print("="*60)

    if GROQ_AVAILABLE:
        print(" Groq API   : Disponible")
    else:
        print("  Groq API   : Non disponible")

    print(" VirusTotal : Actif")
    print(" WHOIS      : Actif")
    print("="*60 + "\n")
